...
(() => {
  ...
  function renderCards(cards){
    ...
    const onDown = (clientX) => { down = true; moved = false; startX = clientX; };
  ...
  function wireActions(data){
    ...
    // NEW: Senxia logo click opens modal
    const senxiaBtn = $("#btnOpenSenxia");
    if (senxiaBtn){
      senxiaBtn.addEventListener("click", () => openModal("senxiaModal"));
    }

    // NEW: Showcase button action (edit URL below)
    const showcaseBtn = $("#btnSenxiaShowcase");
    if (showcaseBtn){
      showcaseBtn.addEventListener("click", () => {
        // Put your checkout/contact link here:
        const showcaseUrl = "https://example.com/showcase-the-journey";
        safeOpen(showcaseUrl);
      });
    }
  ...
  function startFx(){
    ...
    function drawConstellation(opacityMult){
      drawPass(1.8, 0.35 * opacityMult);
      drawPass(1.0, 1.0 * opacityMult);
    }

    function drawPass(sizeMult, alphaMult){
      for (const s of p){
